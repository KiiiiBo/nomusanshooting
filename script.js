...
window.onload = function () {
	...
	game.onload = function () {					//ロードが終わった後にこの関数が呼び出されるので、この関数内にゲームのプログラムを書こう
		...
		zoyamaImg.moveTo(160, 160);						//ぞう山ボタンの位置
		...
		game.onenterframe = function () {
			...
			if (zoyamaImg.x >= 400) {						//画面端にぞう山画像が行ってしまったら
				...
				//ゲームオーバー後のテキスト表示
				gameOverText.text = "GAMEOVER!!  記録：" + point + "のむさんポイント";				//テキストに文字表示 
			}
		...
		tweetBtn.ontouchend = function () {				//S_Tweetボタンをタッチした（タッチして離した）時にこの中の内容を実行する
			...
			const url = encodeURI("https://microsoftofficial.github.io/nomusanshooting/");
			window.open("http://twitter.com/intent/tweet?text=頑張って" + point + "のむさんポイント入手した&hashtags=のむさんシューティング&url=" + url); //ハッシュタグにahogeタグ付くようにした。
		};
